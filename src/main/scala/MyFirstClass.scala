...
object Plane {
  def apply(name: String = "Airplane", destination: String = "Haasenbau") =
    new Plane(name, destination)
  def unapply(p: Plane): Option[(String, String)] =
    Some((p.name, p.destination))
}
...
class Plane(val name: String, val destination: String) {
  ...
  def copy(name: String = name, destination: String = destination) =
    Plane(name, destination)
}
